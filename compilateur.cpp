// Done by: SEKOUANE Amel
//Basé sur le compilateur donné par le prof 
...
map<string, enum TYPES> MesVariables;	
unsigned long long TagNumber=0;
unsigned long long CaseN=0;

bool IsDeclared(const char *id){
	return MesVariables.find(id)!=MesVariables.end();
}
...
enum TYPES Identifier(void){
	...
	type=MesVariables[lexer->YYText()];
...
//Carre <integer>  Carre de x = x*x
void Carre(void) {
    // Lire le prochain token 
    current = (TOKEN) lexer->yylex();
    
    // Récupérer l'identifiant
    string iden = lexer->YYText();
    
    // Évaluer l'expression (qui doit être une valeur entiere)
    TYPES type;
    type = Expression();
    
    // Vérifier que l'expression est un entier
    if (type != INTEGER) {
        Error("Le type de l'expression n'est pas un INTEGER");
        return;
    }
    cout << "\tmovq " << iden << ", %rax \t# Charger la valeur de " << iden << " dans le registre %rax" << endl;
    cout << "\timul %rax, %rax \t# Multiplier la valeur par elle-même pour obtenir le carré" << endl;
    cout << "\tmovq %rax, "<< iden << "\t# Stocker le carré de la valeur dans " << iden << endl;
}



// ForStatement := "For" ID ":=" Expression ("TO"|"DOWNTO") Expression "DO" Statement
// ID ":=" Expression == AssignementStatement
// AssignementStatement := Identifier ":=" Expression
void ForStatement(void){
	unsigned long long NbFor=TagNumber++;
	current=(TOKEN) lexer->yylex();
	// stocker identifiant de For
	string iden = lexer->YYText();
	// compteur <- cette valeur
	AssignementStatement();
	// stocker le KEYWORD: to ou downto
	string keyword = lexer->YYText();
	cout<<"For"<<NbFor<<":"<<endl;
	if((current!=KEYWORD||strcmp(lexer->YYText(),"TO")!=0) && (current!=KEYWORD||strcmp(lexer->YYText(),"DOWNTO")!=0))
		Error("  Keyword 'TO' or 'DOWNTO' attendu !!!");
	// le token suivant
	current=(TOKEN) lexer->yylex();
	// calculer l'expression et tester si c entier
	if(Expression()!=INTEGER)
		Error("Nombre entier  attendu !!!");
	// compare la valeur du cpt avec le res
	cout<<"\tpop %rax\t# Le résultat de l'expression"<<endl;
	cout<<"\tcmpq "<<iden<<", %rax"<<endl;
	// sauter à la fin de for si l'identifiant est SUP
	if (strcmp(keyword.c_str(),"TO") == 0) {
		cout<<"\tjb EndFor"<<NbFor<<"\t# if FALSE, quit FOr"<<NbFor<<endl;
	}
	// si c'est MIN aussi
	if (strcmp(key_word.c_str(),"DOWNTO") == 0){
		cout<<"\tja EndFor"<<NbFor<<"\t# if FALSE, quit for"<<NbFor<<endl;
	}
	if(current!=KEYWORD||strcmp(lexer->YYText(), "DO")!=0)
		Error("Keyword  'DO' attendu !!!");
	current=(TOKEN) lexer->yylex();
	Statement();
	// test si TO, incremente
	if (strcmp(keyword.c_str(),"TO") == 0) {
		// incremente
		cout << "\tpush "<<iden<<endl;
		cout << "\tpush $"<<1<<endl;
		cout << "\tpop %rbx"<<endl;	//op1
		cout << "\tpop %rax"<<endl;	// op2
		cout << "\taddq	%rbx, %rax\t# addition"<<endl;	// op1+op2
		cout << "\tpush %rax"<<endl;			//enregistrer
		cout << "\tpop "<<iden<<endl;	// enregistrer dans identifieur
	}
	// test si DOWNTO, decremente
	if (strcmp(keyword.c_str(),"DOWNTO") == 0){
		cout << "\tpush "<<iden<<endl;
		cout << "\tpush $"<<1<<endl;
		cout << "\tpop %rbx"<<endl;	
		cout << "\tpop %rax"<<endl;	
		cout << "\tsubq	%rbx, %rax\t# addition"<<endl;	
		cout << "\tpush %rax"<<endl;		
		cout << "\tpop "<<iden<<endl;	
	}
	cout << "\tjmp For"<<NbFor<<endl;
	cout << "EndFor"<<NbFor<<":"<<endl;
}
...
void CaseLabelList(void) {
    // Tag unique pour chaque cas
    unsigned long long caseTag = TagNumber++;
    
    // Traitement de la première étiquette de cas
    cout << lexer->YYText() << caseTag << ":" << endl; // Affichage de l'étiquette de cas
    cout << "\tpop %r12\t# Récupérer le résultat du registre" << endl;
    cout << "\tpush %rax\t# Sauvegarder le résultat" << endl;
    cout << "\tcmpq " << lexer->YYText() << ", %rax" << endl; // Comparaison avec la valeur dans %rax
    cout << "\tjne EndCase" << CaseN << "\t# Sauter à la fin du cas si la comparaison échoue" << endl;

    // Passage à l'étiquette de cas suivante s'il y en a une
    current = (TOKEN) lexer->yylex();
    while (current == COMMA) {
        current = (TOKEN) lexer->yylex(); // Ignorer la virgule
        // Traitement des étiquettes de cas suivantes
        cout << lexer->YYText() << caseTag << ":" << endl;
        cout << "\tcmpq " << lexer->YYText() << ", %rax" << endl;
        cout << "\tjne EndCase" << CaseN << "\t# Sauter à la fin du cas si la comparaison échoue" << endl;
        current = (TOKEN) lexer->yylex(); // Passage à l'étiquette de cas suivante
    }
}

// CaseListElement ::= CaseLabelList : Statement
void CaseListElement(void) {
    // Traitement de la liste d'étiquettes de cas
    CaseLabelList();

    // Vérification de la présence du symbole ':' après les étiquettes de cas
    if (current != COLON)
        Error("Un ':' est attendu après les étiquettes de cas !!!");
    current = (TOKEN) lexer->yylex();

    // Traitement du bloc de code associé à la liste de cas
    Statement();

    // Marquage de la fin du cas
    cout << "EndCase" << CaseN << ":" << endl;
}
...
void CaseStatement(void) {
    ...
    current = (TOKEN) lexer->yylex();
    Expression();
    cout << "\tpop %rax\t# Récupérer le résultat de l'expression" << endl;
    cout << "\tmovq %rax, %r12\t# Enregistrement de la valeur retournée" << endl;

    // Vérification de la présence du mot-clé "OF" après l'expression
    if (current != KEYWORD || strcmp(lexer->YYText(), "OF") != 0)
        Error("Le mot-clé 'OF' est attendu après l'expression !!!");
    current = (TOKEN) lexer->yylex();

    // Traitement des éléments de la liste de cas
    CaseListElement();
    while (current == SEMICOLON) {
        current = (TOKEN) lexer->yylex(); // Ignorer le ";"
        CaseListElement(); // Traitement du prochain élément de la liste de cas
    }

    // Vérification de la présence du mot-clé "END" à la fin de l'instruction CASE
    if (current != KEYWORD || strcmp(lexer->YYText(), "END") != 0)
        Error("Le mot-clé 'END' est attendu à la fin de l'instruction CASE !!!");
    current = (TOKEN) lexer->yylex();
}
...
void Statement(void){
	if(current==KEYWORD){
		if(strcmp(lexer->YYText(),"DISPLAY")==0)
			...
			CaseStatement();
		else if(strcmp(lexer->YYText(),"CARRE")==0)
			Carre();
...
main(void){	// First version : Source code on standard input and assembly code on standard output
